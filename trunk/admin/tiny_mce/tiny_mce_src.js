...
(function(win){var whiteSpaceRe=/^\s*|\s*$/g,undefined;var tinymce={majorVersion:'3',minorVersion:'3.7',releaseDate:'2010-06-10',_init:function(){var t=this,d=document,na=navigator,ua=na.userAgent,i,nl,n,base,p,v;t.isOpera=win.opera&&opera.buildNumber;t.isWebKit=/WebKit/.test(ua);t.isIE=!t.isWebKit&&!t.isOpera&&(/MSIE/gi).test(ua)&&(/Explorer/gi).test(na.appName);t.isIE6=t.isIE&&/MSIE [56]/.test(ua);t.isGecko=!t.isWebKit&&/Gecko/.test(ua);t.isMac=ua.indexOf('Mac')!=-1;t.isAir=/adobeair/i.test(ua);t.isIDevice=/(iPad|iPhone)/.test(ua);if(win.tinyMCEPreInit){t.suffix=tinyMCEPreInit.suffix;t.baseURL=tinyMCEPreInit.base;t.query=tinyMCEPreInit.query;return;}
...
return u+v;return u.replace('#',v+'#');}};tinymce._init();win.tinymce=win.tinyMCE=tinymce;})(window);(function($,tinymce){var is=tinymce.is,attrRegExp=/^(href|src|style)$/i,undefined;if(!$)
...
sc.parentNode.insertBefore(marker,sc);ieRng.moveToElementText(marker);marker.parentNode.removeChild(marker);if(so>0)
ieRng.move('character',so);}else{ieRng.moveToElementText(sc);if(skipStart)
ieRng.collapse(FALSE);}
if(sc==ec&&sc.nodeType==3){try{ieRng.moveEnd('character',eo-so);ieRng.select();ieRng.scrollIntoView();}catch(ex){}
return;}
...
ieRng.select();ieRng.scrollIntoView();};this.getRangeAt=function(){if(!range||!tinymce.dom.RangeUtils.compareRanges(lastIERng,selection.getRng())){range=getRange();lastIERng=selection.getRng();}
...
node.innerHTML='<br _mce_bogus="1" />';return node;};restoreEndPoint('start');restoreEndPoint('end');rng=dom.createRng();rng.setStart(addBogus(startContainer),startOffset);rng.setEnd(addBogus(endContainer),endOffset);t.setRng(rng);}else if(bookmark.name){t.select(dom.select(bookmark.name)[bookmark.index]);}else if(bookmark.rng)
...
addUndo();});t.onKeyDown.add(function(ed,e){var rng,tmpRng,parent,offset;if(isIE&&e.keyCode==46){rng=t.selection.getRng();if(rng.parentElement){parent=rng.parentElement();tmpRng=rng.duplicate();tmpRng.moveToElementText(parent);tmpRng.setEndPoint('EndToEnd',rng);offset=tmpRng.text.length;if(e.ctrlKey){rng.moveEnd('word',1);rng.select();}
t.selection.getSel().clear();if(rng.parentElement()==parent){try{parent.innerHTML=parent.innerHTML;}catch(ex){}
tmpRng.moveToElementText(parent);tmpRng.collapse();tmpRng.move('character',offset);tmpRng.select();}
...
t._previousFormats=cloneFormats(ed.selection.getStart());});ed.onKeyUp.add(function(ed,e){if(e.keyCode==13&&!e.shiftKey){var parent=ed.selection.getStart(),fmt=t._previousFormats;if(!parent.hasChildNodes()){parent=dom.getParent(parent,dom.isBlock);if(parent){parent.innerHTML='';if(t._previousFormats){parent.appendChild(fmt.wrapper);fmt.inner.innerHTML='\uFEFF';}else
parent.innerHTML='\uFEFF';selection.select(parent,1);ed.getDoc().execCommand('Delete',false,null);}}}});}
